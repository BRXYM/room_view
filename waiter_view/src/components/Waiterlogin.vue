...
const { statusCode, code, waiterList, message } = storeToRefs(waiterStore)
...
function LoginUser() {
  axios.post("http://localhost:3000/waiter/login", {
    "waiter_id": login.value.ID,
    ...
    response => {
      if (response.data.code === "001") {
        ...
        getAllWorking()
        ElNotification({
          title: waiterStore.waiterList.waiter_name,
          message: '登陆成功，欢迎使用',
          type: 'success',
        })
